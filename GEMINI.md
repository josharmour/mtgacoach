...
### Python Coaching Engine ([src/arenamcp/](file:///Z:/mtgacoach/src/arenamcp))

#### Communication & Protocols
* [pipe_adapter.py](file:///Z:/mtgacoach/src/arenamcp/pipe_adapter.py) — Establishes the JSON-over-stdio protocol used by the headless coaching subprocess to communicate back to the PySide6 desktop frontend.
* [gre_bridge.py](file:///Z:/mtgacoach/src/arenamcp/gre_bridge.py) — Runs a local Named Pipe server (`\\.\pipe\mtgacoach_bridge_v2`) to exchange game snapshots and execute automation actions with the BepInEx Unity plugin.
* [gre_serializer.py](file:///Z:/mtgacoach/src/arenamcp/gre_serializer.py) — Protobuf JSON translator for GRE events.
* [gre_action_matcher.py](file:///Z:/mtgacoach/src/arenamcp/gre_action_matcher.py) — Maps high-level decisions from the action planner to low-level GRE action objects.
* [parser.py](file:///Z:/mtgacoach/src/arenamcp/parser.py) & [watcher.py](file:///Z:/mtgacoach/src/arenamcp/watcher.py) — File watcher and parser for `Player.log` acting as a fallback data source.

#### State & Decision Engine
* [gamestate.py](file:///Z:/mtgacoach/src/arenamcp/gamestate.py) — Main game state management. Processes direct GRE bridge protobuf messages and log parsing.
* [coach.py](file:///Z:/mtgacoach/src/arenamcp/coach.py) — Prompt building, LLM querying, advice post-processing, and fallback deterministic scoring logic.
* [action_planner.py](file:///Z:/mtgacoach/src/arenamcp/action_planner.py) — Autonomous gameplay planning using LLM-generated actions.
* [autopilot.py](file:///Z:/mtgacoach/src/arenamcp/autopilot.py) — Core executor for autonomous play, utilizing GRE bridge API and OS level inputs.
* [combat_solver.py](file:///Z:/mtgacoach/src/arenamcp/combat_solver.py) — Solves complex combat math for planning.
* [standalone.py](file:///Z:/mtgacoach/src/arenamcp/standalone.py) — Entry point for headless coaching loop, managing trigger states.

#### Database & External APIs
* [scryfall.py](file:///Z:/mtgacoach/src/arenamcp/scryfall.py) — Scryfall REST API client with local disk cache.
* [draftstats.py](file:///Z:/mtgacoach/src/arenamcp/draftstats.py) & [draft_eval.py](file:///Z:/mtgacoach/src/arenamcp/draft_eval.py) — Queries and processes 17lands data to evaluate draft options.
* [mtgadb.py](file:///Z:/mtgacoach/src/arenamcp/mtgadb.py) & [mtgjson.py](file:///Z:/mtgacoach/src/arenamcp/mtgjson.py) — Integrates local card lookup SQLite databases.

#### Interaction & Desktop Integration
* [input_controller.py](file:///Z:/mtgacoach/src/arenamcp/input_controller.py) — Coordinates direct OS clicks and key input automation.
* [screen_mapper.py](file:///Z:/mtgacoach/src/arenamcp/screen_mapper.py) — Calculates positions on screen for card/button interactions.
* [vision_mapper.py](file:///Z:/mtgacoach/src/arenamcp/vision_mapper.py) — VLM-based pixel-level fallback coordinate resolver.
* [tts.py](file:///Z:/mtgacoach/src/arenamcp/tts.py) & [transcription.py](file:///Z:/mtgacoach/src/arenamcp/transcription.py) — Text-to-Speech synthesis and transcription integration.

### Desktop UI Subsystem ([src/arenamcp/desktop/](file:///Z:/mtgacoach/src/arenamcp/desktop))
* [app.py](file:///Z:/mtgacoach/src/arenamcp/desktop/app.py) — PySide6 application runtime initialization.
* [main_window.py](file:///Z:/mtgacoach/src/arenamcp/desktop/main_window.py) — Main workspace window and tab shell.
* [coach_tab.py](file:///Z:/mtgacoach/src/arenamcp/desktop/coach_tab.py) — Central control panel for voice settings, engine logs, and session controls.
* [repair_tab.py](file:///Z:/mtgacoach/src/arenamcp/desktop/repair_tab.py) — Self-healing diagnostic interface (venv setup, environment settings, BepInEx installation, and bridge validation).
* [hud.py](file:///Z:/mtgacoach/src/arenamcp/desktop/hud.py) & [card_overlay.py](file:///Z:/mtgacoach/src/arenamcp/desktop/card_overlay.py) — Transparent click-through overlay window displaying draft reviews directly on top of MTGA.
* [coach_process.py](file:///Z:/mtgacoach/src/arenamcp/desktop/coach_process.py) — Manages standard subprocess pipes to start/stop the standalone backend process.

### Unity C# Plugin ([bepinex-plugin/MtgaCoachBridge/](file:///Z:/mtgacoach/bepinex-plugin/MtgaCoachBridge))
* [Plugin.cs](file:///Z:/mtgacoach/bepinex-plugin/MtgaCoachBridge/Plugin.cs) — Injected DLL logic. Injects into MTGA Unity lifecycle, handles connection to local Named Pipe (`\\.\pipe\mtgacoach_bridge_v2`), and executes marshaled main-thread queries/actions.
* [BotBattleBridge.cs](file:///Z:/mtgacoach/bepinex-plugin/MtgaCoachBridge/BotBattleBridge.cs) — Intercepts and logs bot match statistics and triggers.

### Proxy Server Backend ([website/](file:///Z:/mtgacoach/website))
