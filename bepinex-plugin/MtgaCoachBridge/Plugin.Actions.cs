    ...
    public partial class Plugin
    {
        ...
        private void HandleGetPendingActions(PipeCommand cmd)
        {
            ...
            else if (request is GroupRequest groupReqExpose)
            {
                ...
                // properties â€” Python gets whatever Min/Max/etc. fields
                // the runtime exposes.
        ...
        {
            // Always get fresh reference â€” cached _lastKnownRequest may have
            ...
            if (request is DeclareBlockersRequest blockersReq)
            {
                ...
                {
                    // No blocks â€” submit empty (like Goldfish bot)
                ...
                else
                {
                    ...
                    if (selectedBlockers.Count > 0)
                    {
                        ...
                        // state â€” calling UpdateBlockers() then SubmitBlockers()
        ...
        private void HandleSubmitAttackers(PipeCommand cmd)
        {
            ...
            if (request is DeclareAttackerRequest attackerReq)
            {
                ...
                {
                    // No attackers specified â€” finalize (submit current declarations).
                ...
                else
                {
                    ...
                    // Step 2: Python calls again with empty list â†’ SubmitAttackers() to finalize.
                    ...
                    {
                        // Requested IDs not found or already declared â€” finalize
        ...
        private void HandleSubmitTargets(PipeCommand cmd)
        {
            ...
            if (request is SelectTargetsRequest targetsReq)
            {
                ...
                // requests â€” e.g. an Aura that enchants your creature AND exiles
                ...
                // left the other slot empty â†’ MTGA rejected â†’ the request
                ...
                // request â€” autopilot then loops forever (issue observed live
                ...
                foreach (var ts in targetsReq.TargetSelections)
                {
                    ...
                    if (chosen == null)
                    {
                        // Slot has zero free legal targets â€” skip; SubmitTargets
                    ...
                    // request's shared _outboundMessage and calls Submit() â€”
                    ...
                    // directly â€” we mirror that pattern here. LegalAction =
                ...
                // making SubmitTargetsReq's GameStateId stale â€” observed live
                ...
                {
                    _log.LogInfo("All targets already selected â€” sending SubmitTargetsReq only");
                    var commitMsg = new ClientToGREMessage
                    {
                ...
                if (finalize && allRequiredFilled)
                {
                    ...
                    // request (SelectTargetsWorkflow.ApplyInteractionInternal â†’
                    // CanAutoSubmitTargets â†’ SubmitTargets). Committing here
                    ...
                    // Humiliation / Swords to Plowshares â€” always with 2+
                ...
                {
                    _log.LogWarning($"SubmitTargets: NOT finalizing â€” only {requiredFilled}/{requiredSlots} required slots filled; leaving request open for retry");
                }
        ...
        /// click â†’ server round-trip â†’ auto-submit sequence.
        /// </summary>
            ...
            int requiredFilled)
        {
            ...
            // full window before the (correct) legacy commit fired â€” observed
            ...
            while (Time.unscaledTime - start < timeoutS)
            {
                ...
                {
                    // Request no longer presented â€” either the client already
                    ...
                    {
                        _log.LogInfo("DeferredSubmitTargets: request no longer pending â€” selection consumed");
                        lock (_interactionLock) { _lastKnownRequest = null; }
            ...
            // here â€” this is the single-phase shape that historically worked.
            ...
            _log.LogWarning(
                ...
                $"{timeoutS:0.0}s â€” committing with last-seen ids " +
                $"(gameStateId={commitTarget.OriginalMessage.GameStateId})");
        ...
        private void HandleSubmitSelectReplacement(PipeCommand cmd)
        {
            ...
            // Payload: {"index": <int>}  â€” index into Replacements list.
        ...
        private void HandleSubmitGather(PipeCommand cmd)
        {
            ...
            // Each Gathering submission is a (target instance, amount) pair â€”
        ...
        private void HandleSubmitAutoTap(PipeCommand cmd)
        {
            ...
            // "Auto Pay" button does â€” auto_respond on the parent might
            ...
            if (autoTapReq == null)
            {
                ...
                // route â€” an ActionsAvailableRequest of mana activations).
            ...
            for (int childIndex = 0; childIndex < request.ChildRequests.Count; childIndex++)
            {
                ...
                switch (child)
                {
                    ...
                    case CastingTimeOption_NumericInputRequest variableNumericReq:
                        // Min != Max â€” enumerate up to MaxNumericInputEntries values so
                    ...
                    case CastingTimeOption_Replicate variableReplicateReq:
                        // Min != Max â€” enumerate replicate counts.
        ...
        // -------------------------------------------------------------------
        // resolve_grp_ids â€” name resolution through the client itself.
