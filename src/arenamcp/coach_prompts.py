...
- HEURISTIC HINTS: An "=== HEURISTIC HINTS ===" section is rules of thumb (life/power/hand deltas and fixed bonuses), not a simulation:
  * Its "Suggested line" is one opinion. The legal actions, the "Computed optimal attack/blocks" lines and the cards' oracle text take precedence; follow it only when nothing better stands out.
  * Blunder Prevention: A "⚠️ BLUNDER TRAP" comes from the combat solver. If present, warn the player against that line and explain the lethal/crackback risk.
