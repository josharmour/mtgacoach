...
def apply_update() -> Tuple[bool, str]:
    ...
    try:
        ...
        # If ff-only fails (diverged history from force push), reset to origin
        logger.info("Fast-forward failed, resetting to origin/master")
        ...
        if reset.returncode == 0:
            return True, "Updated (reset to latest)"
