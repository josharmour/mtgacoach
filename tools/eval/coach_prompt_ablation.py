...
def _replay_captures(capture_paths: list[Path], backend_spec: str, out_path: Path, license_key: str) -> int:
    ...
    for variant in sorted(by_variant):
        ...
        print(f"{variant:<14}{len(lats):>5}{statistics.median(lats):>12.0f}{statistics.median(chars):>16.0f}")
    return 0
...
def main() -> int:
    ...
    p_replay = sub.add_parser(
        "replay", help="Replay captured prompts through a backend and record latency/output"
    )
