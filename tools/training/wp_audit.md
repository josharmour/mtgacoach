...
| Atomic pointer flip in `registry.sqlite` | **tools/training/registry.py:323-336** — `_exec_tx()` drives explicit BEGIN/COMMIT/ROLLBACK for atomic SQLite update; **registry.py:338-385** — `promote_champion()` commits DB first (single tx), then temp file + fsync + `os.replace()` for pointer. Old pointer stays valid through entire operation. **registry.py:128-156** — SQLite schema with `models` table |
...
| Accept: Promote → rollback → promote cycle leaves artifacts intact and pointers clean | **tests/test_model_registry.py:174-262** — `test_promote_rollback_promote_cycle()` verifies promote A → promote B (rollback A) → promote C, all artifacts intact at every step. **tests/test_registry_promotion.py:93-135** — same cycle with `_assert_artifacts_intact()` + `_assert_pointers_clean()` at each phase. Crash safety (DB update atomicity) at **test_model_registry.py:302-360** — `test_database_update_transaction_is_atomic()` simulates crash between two UPDATEs, asserts ROLLBACK keeps old champion. Both suites pass (29 tests, 0.20s). |

**Tests:**
| **tests/test_model_registry.py** (452 lines, 17 tests) — content-addressing, PASS-only, non-clobber, promote/rollback cycle, crash safety (simulated os.replace failure, DB-update atomicity via monkeypatched _exec_tx), retention (keeps last 10, never prunes champion), migration of legacy registries.
